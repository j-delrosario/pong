...
BatchColourLayer::BatchColourLayer()
...
void BatchColourLayer::OnAttach()
{
	EnableGLDebugging();
	
	m_Shader = std::unique_ptr<Shader>(Shader::FromGLSLTextFiles(
		"assets/shaders/BatchColour.vert.glsl",
		"assets/shaders/BatchColour.frag.glsl"
	));
...
void BatchColourLayer::OnDetach()
{
	// Shutdown here
}

void BatchColourLayer::OnEvent(Event& event)
{
	m_CameraController.OnEvent(event);
}

void BatchColourLayer::SetUniformMat4(uint32_t shader, const char* name, const glm::mat4& matrix)
{
	int loc = glGetUniformLocation(shader, name);
	glUniformMatrix4fv(loc, 1, GL_FALSE, glm::value_ptr(matrix));
}

void BatchColourLayer::OnUpdate(Timestep ts)
{
...
void BatchColourLayer::OnImGuiRender()
{
	// ImGui here
}
